...
- [x] 1.3 Say in the proposal that a nested destination stays supported, so the text is not read
      as forbidding what the code allows

## 2. Checks

- [x] 2.1 `openspec validate --changes --strict`
- [x] 2.2 `npm test` unchanged from the base, this being a specification-only change
- [x] 2.3 Confirm no other in-force requirement names a default store path
